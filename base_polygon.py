...
from constants import W, H, SCREEN_AREA
...
class BasePolygon(Polygon):
    ...
    # Original Polygon move method returns convex hull of polygon translatated
    # by (x, y), which transforms a polygon which was convex in a concave one.
    def move(self, x, y):
        """return a new polygon moved by x, y"""
        # return Polygon([(x + p_x, y + p_y) for (p_x, p_y) in self.P])
        return Polygon([(x + p_x, y + p_y) for (p_x, p_y) in self.P], False)
    ...
    def wrap(self):
        ...
        if self.collidepoly(SCREEN_AREA) is False:
            ...
            if self.C[0] < 0 and self.velocity.x < 0:
                self.move_ip(W - min(self.P, key=lambda x: x[0])[0], 0)
            elif self.C[0] > W and self.velocity.x > 0:
                self.move_ip(-max(self.P, key=lambda x: x[0])[0], 0)
            elif self.C[1] < 0 and self.velocity.y < 0:
                self.move_ip(0, H - min(self.P, key=lambda x: x[1])[1])
            elif self.C[1] > H and self.velocity.y > 0:
